...
function DessertInfo() {
  ...
  const getAllDesserts = async () => {
    setloading(true);
    try {
      const productTemp = await getDoc(doc(fireDb, "dessertPost", params.id))
      if (productTemp.exists()) {
        setGetDesserts(productTemp.data());
      } else {
  ...
  const addComment = async () => {
    const userRef = collection(fireDb, "dessertPost/" + `${params.id}/` + "comment")
    try {
      await addDoc(
        userRef, {
        fullName,
  ...
  const getcomment = async () => {
    try {
      const q = query(
        collection(fireDb, "dessertPost/" + `${params.id}/` + "comment/"),
        orderBy('time')
      );
            ...
            <div>
              ...
              <div className="content">
                ...
                className={`[&> h1]:text-[32px] [&>h1]:font-bold  [&>h1]:mb-2.5
                        ...
                        [&>img]:rounded-lg
                        `}
                  dangerouslySetInnerHTML={createMarkup(getDesserts?.desserts?.content)}>
                </div>
              </div>
            </div>
          }
