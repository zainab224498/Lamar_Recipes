...
function SnackInfo() {
  ...
  //* getSnacks State 
  const [getSnacks, setGetSnacks] = useState();

  const getAllSnacks = async () => {
    setloading(true);
    try {
      const productTemp = await getDoc(doc(fireDb, "snackPost", params.id))
      if (productTemp.exists()) {
        setGetSnacks(productTemp.data());
      } else {
  ...
  const addComment = async () => {
    const userRef = collection(fireDb, "snackPost/" + `${params.id}/` + "comment")
    try {
      await addDoc(
        userRef, {
        fullName,
  ...
  const getcomment = async () => {
    try {
      const q = query(
        collection(fireDb, "snackPost/" + `${params.id}/` + "comment/"),
        orderBy('time')
      );
            ...
            <div>
              ...
              <div className="content">
                ...
                className={`[&> h1]:text-[32px] [&>h1]:font-bold  [&>h1]:mb-2.5
                        ...
                        [&>img]:rounded-lg
                        `}
                  dangerouslySetInnerHTML={createMarkup(getSnacks?.snacks?.content)}>
                </div>
              </div>
            </div>
          }
