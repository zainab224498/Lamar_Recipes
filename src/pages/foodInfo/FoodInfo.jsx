...
function FoodInfo() {
  ...
  const getAllFoods = async () => {
    setloading(true);
    try {
      const productTemp = await getDoc(doc(fireDb, "foodPost", params.id))
      if (productTemp.exists()) {
        setGetFoods(productTemp.data());
      } else {
  ...
  const addComment = async () => {
    const userRef = collection(fireDb, "foodPost/" + `${params.id}/` + "comment")
    try {
      await addDoc(
        userRef, {
        fullName,
  ...
  const getcomment = async () => {
    try {
      const q = query(
        collection(fireDb, "foodPost/" + `${params.id}/` + "comment/"),
        orderBy('time')
      );
            ...
            <div>
              ...
              <div className="content">
                ...
                className={`[&> h1]:text-[32px] [&>h1]:font-bold  [&>h1]:mb-2.5
                        ...
                        [&>img]:rounded-lg
                        `}
                  dangerouslySetInnerHTML={createMarkup(getFoods?.foods?.content)}>
                </div>
              </div>
            </div>
          }
