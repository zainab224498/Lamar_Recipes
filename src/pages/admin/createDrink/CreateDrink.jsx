...
function CreateDrink() {
    ...
    //* Add Post Function 
    const addPost = async () => {
        if (drinks.title === "" || drinks.category === "" || drinks.content === "" || drinks.thumbnail === "") {
            toast.error('Please Fill All Fields');
        }
        // console.log(drinks.content)
        uploadImage()
    }
    ...
    const uploadImage = () => {
        if (!thumbnail) return;
        const imageRef = ref(storage, `drinkimage/${thumbnail.name}`);
        uploadBytes(imageRef, thumbnail).then((snapshot) => {
            getDownloadURL(snapshot.ref).then((url) => {
                const productRef = collection(fireDb, "drinkPost")
                try {
                    addDoc(productRef, {
                        drinks,
                        thumbnail: url,
            ...
            <div className="p-5" style={{
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="title"
                        onChange={(e) => setDrinks({ ...drinks, title: e.target.value })} 
                        value={drinks.title}
                    />
                </div>
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="category"
                        onChange={(e) => setDrinks({ ...drinks, category: e.target.value })} 
                        value={drinks.category} 
                ...
                <Editor
                    ...
                    onEditorChange={(newValue, editor) => {
                        setDrinks({ ...drinks, content: newValue });
                ...
                <div className="">
                    ...
                    <div className="content">
                        ...
                        [&>img]:rounded-lg
                        `} dangerouslySetInnerHTML={createMarkup(drinks.content)}></div>
                    </div>
                </div>
            </div>
        </div>
