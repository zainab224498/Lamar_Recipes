...
function CreateFood() {
    ...
    //* Add Post Function 
    const addPost = async () => {
        if (foods.title === "" || foods.category === "" || foods.content === "" || foods.thumbnail === "") {
            toast.error('Please Fill All Fields');
        }
        // console.log(foods.content)
        uploadImage()
    }
    ...
    const uploadImage = () => {
        if (!thumbnail) return;
        const imageRef = ref(storage, `foodimage/${thumbnail.name}`);
        uploadBytes(imageRef, thumbnail).then((snapshot) => {
            getDownloadURL(snapshot.ref).then((url) => {
                const productRef = collection(fireDb, "foodPost")
                try {
                    addDoc(productRef, {
                        foods,
                        thumbnail: url,
            ...
            <div className="p-5" style={{
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="title"
                        onChange={(e) => setFoods({ ...foods, title: e.target.value })} 
                        value={foods.title}
                    />
                </div>
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="category"
                        onChange={(e) => setFoods({ ...foods, category: e.target.value })} 
                        value={foods.category} 
                ...
                <Editor
                    ...
                    onEditorChange={(newValue, editor) => {
                        setFoods({ ...foods, content: newValue });
                ...
                <div className="">
                    ...
                    <div className="content">
                        ...
                        [&>img]:rounded-lg
                        `} dangerouslySetInnerHTML={createMarkup(foods.content)}></div>
                    </div>
                </div>
            </div>
        </div>
