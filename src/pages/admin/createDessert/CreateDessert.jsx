...
function CreateDessert() {
    ...
    //* Add Post Function 
    const addPost = async () => {
        if (desserts.title === "" || desserts.category === "" || desserts.content === "" || desserts.thumbnail === "") {
            toast.error('Please Fill All Fields');
        }
        // console.log(desserts.content)
        uploadImage()
    }
    ...
    const uploadImage = () => {
        if (!thumbnail) return;
        const imageRef = ref(storage, `dessertimage/${thumbnail.name}`);
        uploadBytes(imageRef, thumbnail).then((snapshot) => {
            getDownloadURL(snapshot.ref).then((url) => {
                const productRef = collection(fireDb, "dessertPost")
                try {
                    addDoc(productRef, {
                        desserts,
                        thumbnail: url,
            ...
            <div className="p-5" style={{
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="title"
                        onChange={(e) => setDesserts({ ...desserts, title: e.target.value })} 
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="category"
                        onChange={(e) => setDesserts({ ...desserts, category: e.target.value })} 
                        value={desserts.category} 
                ...
                <Editor
                    ...
                    onEditorChange={(newValue, editor) => {
                        setDesserts({ ...desserts, content: newValue });
                ...
                <div className="">
                    ...
                    <div className="content">
                        ...
                        [&>img]:rounded-lg
                        `} dangerouslySetInnerHTML={createMarkup(desserts.content)}></div>
                    </div>
                </div>
            </div>
        </div>
