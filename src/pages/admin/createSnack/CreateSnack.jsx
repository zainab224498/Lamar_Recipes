...
function CreateSnack() {
    ...
    //* Add Post Function 
    const addPost = async () => {
        if (snacks.title === "" || snacks.category === "" || snacks.content === "" || snacks.thumbnail === "") {
            toast.error('Please Fill All Fields');
        }
        // console.log(snacks.content)
        uploadImage()
    }
    ...
    const uploadImage = () => {
        if (!thumbnail) return;
        const imageRef = ref(storage, `snackimage/${thumbnail.name}`);
        uploadBytes(imageRef, thumbnail).then((snapshot) => {
            getDownloadURL(snapshot.ref).then((url) => {
                const productRef = collection(fireDb, "snackPost")
                try {
                    addDoc(productRef, {
                        snacks,
                        thumbnail: url,
            ...
            <div className="p-5" style={{
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="title"
                        onChange={(e) => setSnacks({ ...snacks, title: e.target.value })} 
                        value={snacks.title}
                    />
                </div>
                ...
                <div className="mb-3">
                 ...
                 : 'placeholder-black'}`}
                        ...
                        name="category"
                        onChange={(e) => setSnacks({ ...snacks, category: e.target.value })} 
                        value={snacks.category} 
                ...
                <Editor
                    ...
                    onEditorChange={(newValue, editor) => {
                        setSnacks({ ...snacks, content: newValue });
                ...
                <div className="">
                    ...
                    <div className="content">
                        ...
                        [&>img]:rounded-lg
                        `} dangerouslySetInnerHTML={createMarkup(snacks.content)}></div>
                    </div>
                </div>
            </div>
        </div>
